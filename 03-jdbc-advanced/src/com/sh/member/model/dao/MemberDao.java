...
//			System.out.println(prop);
	...
	public int deleteMember(Connection conn, String id) {
		...
		try(PreparedStatement pstmt = conn.prepareStatement(sql)){
			...
			throw new MemberException("회원 탈퇴 오류!");
		}
		return result;
	}
	...
	public int updateMemberName(Connection conn, String id, String newName) {
		...
		try(PreparedStatement pstmt = conn.prepareStatement(sql)) {
			...
			throw new MemberException("회원 이름 수정 오류!");
		}
		return result;
	}
	...
	public int updateMemberBirthday(Connection conn, String id, Date newBirthday) {
		int result = 0;
		String sql = prop.getProperty("updateMemberBirthday");
		try(PreparedStatement pstmt = conn.prepareStatement(sql)){
			...
			throw new MemberException("회원 생일 수정 오류!");
		}
		return result;
	}
	...
	public int updateMemberEmail(Connection conn, String id, String newEmail) {
		...
		try(PreparedStatement pstmt = conn.prepareStatement(sql)){
			...
			throw new MemberException("회원 이메일 수정 오류!");
		}
		return result;
	}
	...
	public List<Member_del> findFireAll(Connection conn) {
		...
		try(PreparedStatement pstmt = conn.prepareStatement(sql)){
			...
			throw new MemberException("회원탈퇴 출력 오류!!!");
		}
		return members_del;
	}
