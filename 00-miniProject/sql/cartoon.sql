...
insert into charge_cash_log values (seq_charge_cash_log_no.nextval, ?, ?, ? ,default);
    
create sequence seq_charge_cash_log_no;

--delete from users where id = 'leeyeoun';
select * from del_users_log;
select * from users;
select * from charge_cash_log;
select * from genre;
select * from book;
select * from food;
...
from 
    users 
        u join book b 
            on u.favorite_genre = b.book_genre 
where 
    favorite_genre = 'G1';


select * from users u join genre g on u.favorite_genre = g.genre_id;

insert into book values (seq_book_book_no.nextval, ?, ?, ?, ?, ?);

delete from book where book_no = 52;

select 
    u.*
    , g.genre_title
from 
    users u join genre g
     on u.favorite_genre = g.genre_id;
...
update
    users
set
    charge_cash = charge_cash + 10000
    , balance = balance + 10000
    , point = point + (0.05 * 10000)
where
    id = 'woojin';
...
--insert into charge_cash_log values (seq_charge_cash_log_no.nextval, ?, ?, ? ,default)
-- 충전하기
--update users set charge_cash = charge_cash + ?, balance = balance + ?, point = point + (0.05 * ?) where id = ?
...
--update users set charge_cash = charge_cash + ?, balance = balance + ?, point = point + (0.05 * ?) where id = ? 
select * from users;
update users set balance = balance - 20000 where id = 'leeyeoun' and balance > 0;

-- 일반회원 패스워드 수정 쿼리
-- update users set password = ? where id = (select id from users where id = ? and password = ?)
-- 일반 회원 이름 수정 쿼리
-- update users set name = ? where id = (select id from users where id = ? and password = ?)
-- 일반 회원 생일 수정 쿼리
-- update users set birthday = ? where id = (select id from users where id = ? and password = ?)
-- 일반 회원 번호 수정 쿼리
-- update users set phone = ? where id = (select id from users where id = ? and password = ?)
select * from users where id = 'honggd';

select * from users where password = 'mari';
...
-- del_book_log 관련 테이블 & 트리거
create table del_book_log
as
select
b.*
, systimestamp del_date
...
where
  ...
  --trig
 create or replace trigger trig_delete_book
    after
    delete on book
    for each row
begin
...
desc users;
select * from charge_cash_log;
select favorite_genre from users u join genre g on u.favorite_genre = g.genre_id;
